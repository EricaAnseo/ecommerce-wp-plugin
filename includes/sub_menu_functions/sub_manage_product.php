...
function simp_ec_manage_products_page_html()
{
	...
	$results_product_type = $wpdb->get_results( 'SELECT * FROM ' . $table_pv . ' JOIN ' . $table_product . ' ON ' .  $table_product .'.product_id = ' . $table_pv . '.product_id JOIN ' . $table_pt . ' ON ' .  $table_pt .'.ptype_id = ' . $table_pv . '.ptype_id GROUP BY ' . $table_product . '.product_id');
